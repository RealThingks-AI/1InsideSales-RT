...
export function AppSidebar({ isFixed = false, isOpen, onToggle }: AppSidebarProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-center border-b border-sidebar-border relative h-12 px-2">
        <div 
        ...
        >
          ...
          <div 
            className={`ml-2 text-sidebar-foreground font-semibold text-sm whitespace-nowrap transition-all duration-300 overflow-hidden ${
          ...
          {menuItems.map((item) => {
            ...
            const menuButton = (
              <NavLink
                to={item.url}
                className={`
                  flex items-center h-8 rounded-md relative transition-colors duration-200 font-medium
                  ${active 
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={`flex items-center h-8 w-full rounded-md transition-colors font-medium ${
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="flex items-center h-8 w-full rounded-md transition-colors text-sidebar-foreground/70 hover:text-sidebar-primary hover:bg-sidebar-accent/50 font-medium"
              >
                <div className="w-8 h-8 flex items-center justify-center flex-shrink-0">
                  {(() => {
                    const ThemeIcon = getThemeIcon();
                    return <ThemeIcon className="w-3.5 h-3.5" />;
                  })()}
                </div>
                <div 
                  className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${
        ...
        <div>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <div className="w-8 h-8 flex items-center justify-center flex-shrink-0">
                  {sidebarOpen ? <Pin className="w-3.5 h-3.5" /> : <PinOff className="w-3.5 h-3.5" />}
                </div>
                <div 
                  className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${
                    sidebarOpen ? 'opacity-100 w-auto ml-0' : 'opacity-0 w-0 ml-0'
